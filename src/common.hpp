...
struct IMU {
    ...
    IMU(double t, const Eigen::Vector3d& gyro, const Eigen::Vector3d& acce, const Eigen::Vector3d& mag) 
    : timestamp(t), gyro(gyro), acce(acce), mag(mag) {
    }

    IMU(double t, const Eigen::Vector3d& gyro, const Eigen::Vector3d& acce, const Eigen::Vector3d& mag, const Eigen::Vector3d& angle, const Eigen::Quaterniond q) 
    : timestamp(t), gyro(gyro), acce(acce), mag(mag), angle(angle), q(q) {
    }
